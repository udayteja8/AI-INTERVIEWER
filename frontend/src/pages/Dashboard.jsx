...
function Dashboard() {
  ...
  const getInterviewName = (item) => {
    ...
    if (item.questions?.[0]?.topic) return "Topic - " + item.questions[0].topic;
    return "Interview";
  };
      ...
      <div style={styles.mainContent}>
        ...
        {loading ? (
          ...
          <div style={styles.emptyState}>
            <p style={styles.text}>No interviews yet.</p>
                ...
                {item.report && (
                  <div style={styles.scores}>
                    <div style={styles.scoreItem}>
                      ...
                      <span style={styles.scoreValue}>{item.report.overall_score}/100</span>
                    </div>
                    <div style={styles.scoreItem}>
                      <span style={styles.scoreLabel}>Technical</span>
                      <span style={styles.scoreValue}>{item.report.technical_average}/10</span>
                    </div>
                    <div style={styles.scoreItem}>
                      <span style={styles.scoreLabel}>Comm</span>
                      <span style={styles.scoreValue}>{item.report.communication_average}/10</span>
                    </div>
                  </div>
                )}
          ...
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.modalBody}>
              <div style={styles.modalScores}>
                <div style={styles.modalScoreItem}><span>Overall</span><b>{selectedReport.report?.overall_score}/100</b></div>
                <div style={styles.modalScoreItem}><span>Technical</span><b>{selectedReport.report?.technical_average}/10</b></div>
                <div style={styles.modalScoreItem}><span>Communication</span><b>{selectedReport.report?.communication_average}/10</b></div>
              </div>

              {selectedReport.answers?.map((ans, i) => (
                <div key={i} style={styles.qCard}>
                  <p style={styles.qNum}>Q{i + 1}: {ans.question}</p>
                  <p style={styles.aText}><i>"{ans.answer}"</i></p>
                  
                  {ans.evaluation && (
                    <div style={styles.evalBox}>
                      <div style={styles.evalScores}>
                        <span>Technical: {ans.evaluation.technical_score}/10</span>
                        <span>Communication: {ans.evaluation.communication_score}/10</span>
                      </div>
                      {ans.evaluation.correct_answer && (
                        <p style={styles.correctAnswer}><strong>Correct Answer:</strong> {ans.evaluation.correct_answer}</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              {selectedReport.report?.strengths && (
                <div style={styles.summaryRow}>
                  <strong style={{color: "black"}}> Strengths:</strong>
                  <p>{selectedReport.report.strengths.join(", ")}</p>
                </div>
              )}
              {selectedReport.report?.weak_topics && (
                <div style={styles.summaryRow}>
                  <strong style={{color: "black"}}>Weak Topics:</strong>
                  <p>{selectedReport.report.weak_topics.join(", ")}</p>
                </div>
              )}
              {selectedReport.report?.summary && (
                <div style={styles.summaryBox}>
                  <strong> Summary:</strong>
                  <p>{selectedReport.report.summary}</p>
                </div>
              )}
            </div>
          </div>
...
const styles = {
  ...
  text: { textAlign: "center", color: "#666", fontSize: "14px" },
  ...
  interviewName: { fontSize: "16px", fontWeight: 700, color: "#000", margin: 0 },
  ...
  detailBtn: { width: "100%", padding: "10px", background: "#555555", color: "#fff", border: "none", borderRadius: "6px", fontSize: "13px", fontWeight: 600, cursor: "pointer" },
  ...
  modalScores: { display: "flex", gap: "12px", marginBottom: "20px" },
  ...
  summaryRow: { marginBottom: "10px", fontSize: "13px" },
